...
public class UserController {
    ...
    public BaseResponse<User> userLogin(@RequestBody UserLoginRequest userLoginRequest, HttpServletRequest request) {
        ...
        User userInfo = userService.userLogin(userLoginRequest, request);
     ...
     */
    @PostMapping("/update")
    public BaseResponse<Integer> updateUser(@RequestBody UserEditRequest user, HttpServletRequest request) {
        // 1. 校验参数是否为空
        if (user == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
